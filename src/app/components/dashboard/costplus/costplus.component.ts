...
@Component({
  selector: 'app-costplus',
  templateUrl: './costplus.component.html',
  styleUrls: ['./costplus.component.scss']
})
export class CostplusComponent implements OnInit {
  ...
  public tituloTabla: string="Portafolio Cost Plus";
...
filterValue: filterValues;
 ...
 public filtro: string;
  ...
  public getProductos(sociedad:number): void {
    this.portafolioMoliendaService.getproductos_cost(sociedad).subscribe(
      (response: Underlying[]) => {
  ...
  public getCompanias(): void {
    this.portafolioMoliendaService.getSociedades_cost().subscribe(
      (response: tipoSociedad[]) => {
        this.compania = response;console.log(this.compania)
      },
      (error: HttpErrorResponse) => {
        alert(error.message);
      });
      ...
      (response: SalesContract) => {
        ...
        this.portafolioMoliendaService.obtenerPriceMonth(this.contextMenu.menuData.item).subscribe(
          (response: PriceMonth) => {
             ...
             if(this.companiaSelected.toString() == "22" && this.nuevoContrato.t218_GrindingCustomer == '1' && this.nuevoContrato.t218_SellBuy == '2'){
              if(this.estadoPortafolio.length > 0) {
                Swal.fire({
                  icon: 'warning',
                  title: 'Aviso',
  ...
  this.flgBontonContrato = true;
    ...
    if(this.companiaSelected==21){
      this.nuevoContrato.t218_PortUp="16"
      this.nuevoContrato.t218_PortFrom="16"
    }
    if(this.companiaSelected==21||this.companiaSelected==22){
      this.nuevoContrato.t218_GrindingCustomer="215"
    }
          ...
          (response: string[]) => {console.log(response)
            ...
            this.portafolioMoliendaService.listarComprasEntreCompanias(Number(this.nuevoContrato.t218_GrindingCustomer)).subscribe(
              (response: string) => {console.log(response)
                this.nuevoFisico.t039_Society = Number(response[0]["t231_Society"]);
                this.portafolioMoliendaService.getEstadoCrossCompany(Number(this.nuevoFisico.t039_UnderlyingClassification),this.nuevoFisico.t039_Society).subscribe(
                  (response: ClosingControl[]) => {console.log(response)
                    if(response.length>0){
                      Swal.fire({
                        icon: 'warning',
                        title: 'Aviso',
                      ...
                      this.contratoCross.animalNutrition= this.nuevoNutricionAnimal
                      this.contratoCross.consumoHumano=this.consumoHumano
                      console.log(this.contratoCross)
  ...
  selectCliente(id:number, formularioBarco:any){
    if(this.companiaSelected.toString() !== "22" || this.nuevoContrato.t218_GrindingCustomer !== '1' || this.nuevoContrato.t218_SellBuy !== '2'){
      this.interCompany= false;
      if(this.companiaSelected.toString() == "22" && this.nuevoContrato.t218_GrindingCustomer == '1'){
  ...
  ModalVentaParcial(IngresarVentaParcial:any, ingresarFactura: any){
    ...
    if(fila[0]["s118_Cliente"].toUpperCase().indexOf("ALICONSUMO") >= 0 || this.companiaSelected == 22){
      this.nuevoContrato = new SalesContract();
      ...
      if(this.companiaSelected.toString() == "21"){
        this.portafolioMoliendaService.codigoContrato=this.contextMenu.menuData.item;
  ...
  guardarVentaParcial(IngresarVentaParcial:any){
    ...
    if(this.pasarFacturadaVP == true){
      ...
      if(this.companiaSelected.toString() == "22"){
  ...
  obtenerDescargasParciales(listaVentaParcial:any, listaDosificada:any){
    ...
    if(fila[0]["s118_Cliente"].toUpperCase().indexOf("ALICONSUMO") >= 0){
