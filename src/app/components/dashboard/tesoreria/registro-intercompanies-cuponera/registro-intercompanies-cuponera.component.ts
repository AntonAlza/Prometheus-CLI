...
export class RegistroIntercompaniesCuponeraComponent implements OnInit {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed){
        for(let i of this.listOC){
          this.tesoreriaService.postGuardarCO_Cuponera(i, this.listCuponera).subscribe(
            (response: any) => {
  ...
  envioCorreo(tabla, usuarioSaludo){
    ...
    this.estructuraCorreo.asunto = "[REGISTRO] " 
      ...
      + " por "
      + tabla['Moneda'] + " "
      + Math.trunc(Number(tabla[(this.listOC[0].t454_id_type_co == 'INTER' ? 'Monto' 
    ...
    for(let datosUsuario  of usuarioSaludo.UsuarioSaludo){
