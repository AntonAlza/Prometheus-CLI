...
import { iif, Observable, pipe, Subject, Subscription } from 'rxjs';
...
@Component({
  ...
  selector: 'app-registrosOperacionesBroker',
  templateUrl: './registrosOperacionesBroker.component.html',
  styleUrls: ['./registrosOperacionesBroker.component.scss']

  
})

export class registrosOperacionesBrokerComponent implements OnInit {
  ...
  public disabledRegresarBroker:Boolean=false;
  public disabledGuardarPortafolio:Boolean=false;
  public disabledRegistrar:Boolean=false;
  public disabledAsociar:Boolean=false;
  public disabledLiquidar:Boolean=false;
  public estadoBroker:Boolean=false  
  ...
  public getListaBroker_SQL_Operaciones(empresa: number, fecha:number): void {
    let MtM:number=0;
    let operacionSQL: OperacionesSQL;
    ...
    (response: listaoperacionesBroker_SQL[]) => {
      this.listaOperacionesBroker_SQL = response;
       //agregar MtM operaciones a liquidar.
       for (let indice = 0; indice < this.listaOperacionesBroker_SQL.length; indice++) {
            if (this.listaOperacionesBroker_SQL[indice].s211_estado=='Pre-Liquidación'){
              this.portafolioMoliendaIFDService.getOperacion_SQL(Number(this.listaOperacionesBroker_SQL[indice].s211_IDSQL)).subscribe(
                (response: OperacionesSQL) => {
                  operacionSQL = response;
                  MtM=this.calcularMtM(this.listaOperacionesBroker_SQL[indice],operacionSQL);
                  this.listaOperacionesBroker_SQL[indice].s211_MtM=MtM.toString();
...
validarOperaciones():boolean{
     ...
     for (let operacionBroker of  this.selection.selected ){
        if(  operacionBroker != null )
        {
          if(operacionBroker.s210_CAKS_UTILIZAR>0 )
          {
            ...
            this.operacionBroker_IDSQL.usuario=this.usuario
  ...
  Swal.fire({
      icon: 'warning',
      title: 'Aviso',
      text: 'No ha seleccionado ninguna operación Broker',
...
GuardarOperacionesBroker(){
    ...
    // console.log(this.selection.selected);
  if (Number(this.idBroker)==4 || Number(this.idBroker)==10){ //caso de RJO
      ...
      else
      {
          ...
          else if (this.selection.selected.length>0 && this.validarOperaciones()) {
            this.disabledRegistrar=true;
            this.loader.show();
            this.portafolioMoliendaIFDService.guardarPreregistroSQL(this.listaoperacionBroker_IDSQL).subscribe(
              data=>{
  ...
  else if (Number(this.idBroker)==2){ //caso CRM
    if (this.selection.selected.length==0 ){
      Swal.fire({
        icon: 'error',
        title: 'Aviso',
        text: 'Debe seleccionar alguna operación',
        confirmButtonColor: '#0162e8',
        customClass: {
          container: 'my-swal'
        }
    ...
    {
        if (this.validarOperaciones() && this.validarPrima()) {
          this.disabledRegistrar=true;
          this.loader.show();
          this.portafolioMoliendaIFDService.guardarPreregistroSQL(this.listaoperacionBroker_IDSQL).subscribe(
            data=>{
...
// }
  ...
  ngOnInit(): void {
    ...
    //   let modalContent: any = $('.msg_card_body');
    //   let modalHeader = $('.msg_card_body');
    //   modalHeader.addClass('cursor-all-scroll');
    //   modalContent.draggable({
    //     handle: '.msg_card_body'
    //   });
    // });
      ...
      this.idBroker=this.portafolioMoliendaIFDService.broker
      if (Number(this.idBroker)==2) //2:CRM
      {
         this.estadoBroker=false 
      }
      else if (Number(this.idBroker)==4) //4:RJO
      {
        this.estadoBroker=true
      }  

      this.portafolioMoliendaIFDService.refresh$.subscribe(() => {
          this.getListaBrokerOperaciones(this.portafolioMoliendaIFDService.codigoEmpresa,this.productoselected, this.fecha,Number(this.idBroker));
      ...
      this.getBroker();
      this.disabledRegresarBroker=false;
      this.disabledGuardarPortafolio=false;
      this.disabledRegistrar=false;
      this.disabledAsociar=false;
...
private getDismissReason(reason: any): string {
  ...
  GuardarOperacionesSQL(){
    ...
    this.listaOperacionesBroker_SQLseleccionadas=[];
    ...
    for (let operacionesBroker_SQL of  this.selectionSQL.selected ){
        if(  operacionesBroker_SQL != null  )
        {
          ...
          //   console.log('Valor  Instrumento:'+ operacionesBroker_SQL.s211_Instrumento );
            ...
            this.indice=this.indice+1;
            this.operacionBroker= new listaoperacionesbrokers;
            //this.operacionSQL_Broker.pricing=new Pricing();
            //this.operacionSQL_Broker.pricing=this.portafolioMoliendaIFDService.pricing;
          }
      } // FOR
      this.disabledGuardarPortafolio=true;   
      this.loader.show();
  ...
  modalAsociarPreRegistro(detalleForm1:any){
    ...
    this.portafolioMoliendaIFDService.listaOperacionesBrokerAsociar=[];
    if (this.selectionSoyCrush.selected.length>0 &&     this.selection.selected.length>0 ){
      Swal.fire({
        icon: 'warning',
        title: 'Aviso',
  ...
  modalLiquidarRegistroSQL(detalleForm2:any){
    ...
    this.portafolioMoliendaIFDService.listaOperacionesBrokerLiquidar=[];
    if (this.selectionSoyCrush.selected.length>0 &&     this.selection.selected.length>0 ){
      this.selectionSoyCrush.clear();
      this.selection.clear();
      Swal.fire({
    ...
    else
    {
            ...
            {    this.myModalLiquidar=true;
              this.myModalAsociar=true;
                ...
                if (this.flagLiquidacion) {
                  ...
                  this.modalService.open(detalleForm2,{windowClass : "ventaliquidar"});
            ...
            switch (this.tipoMensaje) {
              case 1: 
                      ...
                      Swal.fire({
                        icon: 'warning',
                        title: 'Aviso',
                        text: 'El futuro o swap tiene que ser similares',
                        confirmButtonColor: '#0162e8',
                        customClass: {
                          container: 'my-swal'
                        }
              ...
              case 2:
                        ...
                        Swal.fire({
                          icon: 'warning',
                          title: 'Aviso',
                          text: 'La opción tiene que tener el mismo strike',
                          confirmButtonColor: '#0162e8',
                          customClass: {
                            container: 'my-swal'
                          }
      ...
      else if (this.selectionSoyCrush.selected.length>0)
      {
        if (this.validarOperacionesSoyCrush()) 
        {    this.myModalLiquidar=true;
          this.myModalAsociar=true;
            ...
            if (this.flagLiquidacion) {
            //this.modalService.open(detalleForm2, { centered: true,size: 'ventaliquidar' });
            this.modalService.open(detalleForm2,{windowClass : "ventaliquidar"});
            }
            //clearInterval();
        }
          ...
          Swal.fire({
            icon: 'warning',
            title: 'Aviso',
            text: 'Revise la operación soycrush',
            confirmButtonColor: '#0162e8',
            customClass: {
              container: 'my-swal'
            }
  ...
  RevertirOperacionesSQL(){

    console.log('Revertir Operacion  Broker - SQL'+this.selectionSQL.selected);
    if (this.selectionSQL.selected.length>0)
    {
    ...
    for (let operacionesBroker_SQL of  this.selectionSQL.selected ){
     if(  operacionesBroker_SQL != null  )
     {
      ...
      this.disabledRegresarBroker=true;   
      this.loader.show();
        
      this.portafolioMoliendaIFDService.revertirOperacion_SQL(this.listaOperacionesBroker_SQLseleccionadas).subscribe(data=>{
          Swal.fire({
            position: 'center',
            icon: 'success',
            title: 'Se revertio la operación',
            showConfirmButton: false,
            timer: 1500,
            customClass: {
            container: 'my-swal',
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Aviso',
      text: 'No ha seleccionado ninguna operación para revertir',
      confirmButtonColor: '#0162e8',
      customClass: {
        container: 'my-swal'
      }
  ...
  public existeOperacionesSimilaresLiquidar(pListaOperacionesBroker: listaoperacionesbrokers[]):boolean{
    ...
    //validar si son opciones el strike sea el  mismo
    if(this.operacionBroker.s210_Instrument==3 || this.operacionBroker.s210_Instrument==4 ){
        //caso de futuro o swap
        if ((pListaOperacionesBroker.filter(i => i.s210_TICKER != this.operacionBroker.s210_TICKER).length>0)
        || (pListaOperacionesBroker.filter(i => i.s210_TIPOOPCION != this.operacionBroker.s210_TIPOOPCION).length>0 )
        ||(pListaOperacionesBroker.filter(i => i.s210_TIPOOPERACION != this.operacionBroker.s210_TIPOOPERACION).length>0)
        )
        ...
        || (pListaOperacionesBroker.filter(i => i.s210_TIPOOPCION != this.operacionBroker.s210_TIPOOPCION).length>0 )
        ||(pListaOperacionesBroker.filter(i => i.s210_TIPOOPERACION != this.operacionBroker.s210_TIPOOPERACION).length>0)
        ||(pListaOperacionesBroker.filter(i => Math.abs(i.s210_STRIKE - this.operacionBroker.s210_STRIKE)>0.001).length>0)
        )
        {
                    this.tipoMensaje=2;
                    return false;
    ...
    onSelectTipoOperacion(detalleForm:any, id,idUnderlying,idSQL){
      if (typeof id !== 'undefined') {
          if (Number(id.value)==3){
              this.portafolioMoliendaIFDService.operacionSQL=new PortafolioIFDMolienda;
              this.portafolioMoliendaIFDService.usuario=this.usuario;
              this.portafolioMoliendaIFDService.fecha=this.fecha;
              this.portafolioMoliendaIFDService.idUnderlying=idUnderlying;
              this.portafolioMoliendaIFDService.idSQL=idSQL;
              //Cargar registro de Pricing
              console.log("modal abierto");
              const modalRef =this.modalService.open(detalleForm,{windowClass : "my-classRegistroPricing"});
          }
          else{
            this.portafolioMoliendaIFDService.getEliminarPricing(idSQL).subscribe(
              data=>{
              },
              (error: HttpErrorResponse) => {
                alert(error.message);
    ...
    validarPrima():boolean{

      var totalPrimaProducto=0;
      var totalPrimaProductoRF=0; 
      this.indice=0;
      this.existeOperacionBroker=false
      this.operacionBroker= new listaoperacionesbrokers;
      if (this.selection.selected.length>0)
      {
         ...
         for (let operacionBroker of  this.selection.selected ){
            if(  operacionBroker != null )
            {
              if(operacionBroker.s210_CAKS_UTILIZAR>0 )
              {
                totalPrimaProducto=totalPrimaProducto+operacionBroker.s210_CAKS_UTILIZAR*operacionBroker.s210_PRIMA*(operacionBroker.s210_SellBuy===1?1:-1);
                totalPrimaProductoRF=totalPrimaProductoRF+operacionBroker.s210_CAKS_UTILIZAR*operacionBroker.s210_PRIMA_FO*(operacionBroker.s210_SellBuy===1?1:-1);
              }
            }
          }
          if (totalPrimaProducto.toFixed(7)==(totalPrimaProductoRF.toFixed(7))){
            this.existeOperacionBroker=true;
          }
          else{
            Swal.fire({
              icon: 'warning',
              title: 'Aviso',
              text: 'La prima total ingresada no es la misma que la del broker',
              confirmButtonColor: '#0162e8',
              customClass: {
                container: 'my-swal'
              }
    ...
    public calcularMtM(operacionBrokerLiquidar: listaoperacionesBroker_SQL,operacionSQL: OperacionesSQL):number{
      let resultado:number=0;  
      let SIGNO:number
      let PRECIO:number=0;
      let PRECIO_SIN_COSTO:number=0;
      let precioUSD:number=0;
      let factorUnitMeasure:number=0;
      
      if (Number(operacionBrokerLiquidar.s211_SellBuy)==1) SIGNO=1
      else SIGNO=-1

      switch (Number(operacionBrokerLiquidar.s211_Instrument)) {
        case 3:   
                  PRECIO =operacionBrokerLiquidar.s211_strike + (SIGNO *operacionBrokerLiquidar.s211_CostUSD);
                  PRECIO_SIN_COSTO=operacionBrokerLiquidar.s211_strike 
                  precioUSD=operacionSQL.t005_StrikePriceUSD;
                  break;
        case 4:   
                  PRECIO =operacionBrokerLiquidar.s211_strike + (SIGNO *operacionBrokerLiquidar.s211_CostUSD);
                  PRECIO_SIN_COSTO=operacionBrokerLiquidar.s211_strike
        ...
        default:  PRECIO =operacionBrokerLiquidar.s211_prima+ operacionBrokerLiquidar.s211_CostUSD
                  //PRECIO_SIN_COSTO=operacionBrokerLiquidar.s211_prima
                  precioUSD=operacionSQL.t005_PremiumUSD;
      }
      

      resultado=operacionBrokerLiquidar.s211_VolumeUnitMeasure*SIGNO*(precioUSD- PRECIO);
