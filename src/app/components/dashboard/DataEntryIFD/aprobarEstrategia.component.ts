...
@Component({
  ...
  selector: 'app-aprobarEstrategia',
  templateUrl: './aprobarEstrategia.component.html',
  styleUrls: ['./aprobarEstrategia.component.scss']
...
// >>>>>>> Dev
export class aprobarEstrategiaComponent implements OnInit {
  ...
  public listaAprobarEstrategia: AprobarEstrategia []=[];
  public aprobarEstrategiaEstado:AprobarEstrategia_Estado;
...
public estadoAdministrador:boolean=false;
  ...
  dataSource: MatTableDataSource<AprobarEstrategia>;
  selection = new SelectionModel<AprobarEstrategia>(true, []);
  hidden = false;
...
public getListaEstrategiaAprobar(): void {
  ...
  this.portafolioMoliendaIFDService.getListaEstrategiaAprobar(Number(this.idEstado),Number(this.pFechaInicio),Number(this.pFechaFin)).subscribe(
  (response: AprobarEstrategia[]) => {
    this.listaAprobarEstrategia = response;
    this.dataSource = new MatTableDataSource(this.listaAprobarEstrategia);

    this.selection = new SelectionModel<AprobarEstrategia>(true, []);
...
noAprobar(fila: AprobarEstrategia):boolean{
  var mensaje:string;
  var estado:number;
  if (fila.temp_Estado=='Por Aprobar'){
      this.portafolioMoliendaIFDService.getRolUsuario(this.usuario).subscribe(
          ...
          mensaje='¿Esta seguro que no desea aprobar la estrategia '
...
aprobar(fila: AprobarEstrategia){
  var mensaje:string;
  var estado:number;
  if (fila.temp_Estado=='Por Aprobar'){
    this.portafolioMoliendaIFDService.getRolUsuario(this.usuario).subscribe(
      (response: CargarCombo[]) => {
      this.rolesUsuario = response;
      mensaje='¿Esta seguro que desea aprobar la estrategia '
...
public validarUsuario(mensaje:string, fila:AprobarEstrategia,estado:number):void{
  ...
  if(this.estadoAprobador){
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Aprobación Estrategia',
      html: mensaje+'<b> '+ fila.temp_GroupOptions   +'?',
      ...
      if (result.isConfirmed) {
        this.aprobarEstrategiaEstado= new AprobarEstrategia_Estado();
        this.aprobarEstrategiaEstado.aprobarEstrategia=fila;
        this.aprobarEstrategiaEstado.estado=estado
        this.aprobarEstrategiaEstado.aprobarEstrategia.temp_approvalUser=this.usuario;
        this.aprobarEstrategiaEstado.aprobarEstrategia.temp_approvalDate=this.fecha;
        this.portafolioMoliendaIFDService.getActualizarEstrategia(this.aprobarEstrategiaEstado).subscribe(data=>{
          Swal.fire({
            position: 'center',
            icon: 'success',
    ...
    eliminarPeticion(fila: AprobarEstrategia){
      var mensaje:string;
      var estado:number;
      if (fila.temp_Estado=='Por Aprobar'){
        this.portafolioMoliendaIFDService.getRolUsuario(this.usuario).subscribe(
          (response: CargarCombo[]) => {
          this.rolesUsuario = response;
          mensaje='¿Esta seguro que desea eliminar la solicitud? '
      ...
      public validarUsuarioELiminarPeticion(mensaje:string, fila:AprobarEstrategia,estado:number):void{
